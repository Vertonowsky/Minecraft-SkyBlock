...
class Przelewy24 {
    ...
    public function trnRegister($redirect = false) {
		...
		$title = $this->postData["p24_transfer_label"];
		$amount = $this->postData["p24_amount"];
		...
		$sql = "INSERT INTO p24_transactions (id, sessionId, date_initialize, amount, title) VALUES (NULL, '$sessionId', '$date', '$amount', '$title')";
		$conn->query($sql);
		$conn->close();
    ...
    public function trnVerify() {
		...
		if(isset($RES["error"]) and $RES["error"]==='0') {
			...
			$sessionId = $this->postData["p24_session_id"];
			$orderId = $this->postData["p24_order_id"];
			$amount = $this->postData["p24_amount"];
			$currency = $this->postData["p24_currency"];
			$error = $RES["error"];
			
			$sql3 = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM p24_transactions WHERE sessionId='$sessionId'"));
			$title = $sql3['title'];
			...
			$sql2 = "INSERT INTO p24_success (id, sessionId, orderId, date_proceed, amount, currency, title, name, received, error) VALUES (NULL, '$sessionId', '$orderId', '$date', '$amount', '$currency', '$title', '$name', 0, '$error')";
			$conn->query($sql2);
